...
fn main() -> Result<()> {
    ...
    rt.block_on(github.repos().iter(&RepoListOptions::builder().affiliation(vec![Affiliation::Owner]).build()).filter(|r| r.fork).fold((), move |(), r| {
        println!();
        ...
        let http_cache = HttpCache::in_home_dir();
        let github: Github<_> = Github::custom(host, agent, credentials, client, http_cache);
        let branches_stream = github.repo(r.owner.login, r.name).branches().iter();
        fn per_branch(b: hubcaps::branches::Branch) {
